...
export default function Profile({ author, social, researchInterests }: ProfileProps) {
    ...
    const interestTags = researchInterests
        ? researchInterests
    ...
    return (
        <motion.div
            ...
            className="sticky top-20 lg:top-24 self-start"
        >
            {/* Profile Image */}
            <div className="w-32 h-32 mx-auto mb-5 rounded-full overflow-hidden shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-105">
                <Image
                    src={author.avatar}
                    alt={author.name}
                    width={256}
            ...
            <div className="text-center mb-5">
                ...
                <p className="text-base text-neutral-500 dark:text-neutral-400 font-medium mb-1">
                    {author.title}
                </p>
                ...
                {/* Contact Links */}
                <div className="flex flex-wrap justify-center gap-1 relative px-2">
                    {socialLinks.map((link) => {
                            ...
                            return (
                                <div key={link.name} className="relative">
                                    <button
                                        ...
                                        className={`p-2 rounded-lg transition-colors duration-200 ${isAddressPinned
                                            ? 'text-primary bg-neutral-100 dark:bg-neutral-800'
                                            : 'text-neutral-500 dark:text-neutral-400 hover:text-primary hover:bg-neutral-100 dark:hover:bg-neutral-800'
                                            }`}
                                                    ...
                                                    <div className="mt-2 flex flex-col space-y-2 sm:flex-row sm:space-y-0 sm:space-x-2 justify-center">
                                                        {social.location_url && (
                                                            <a
                                                                ...
                                                                className="inline-flex items-center justify-center space-x-2 bg-neutral-600 hover:bg-neutral-500 text-white px-3 py-1 rounded-md text-xs font-medium transition-colors duration-200 w-full sm:w-auto"
                                                            >
                            ...
                            return (
                                <div key={link.name} className="relative">
                                    <button
                                        ...
                                        className={`p-2 rounded-lg transition-colors duration-200 ${isEmailPinned
                                            ? 'text-primary bg-neutral-100 dark:bg-neutral-800'
                                            : 'text-neutral-500 dark:text-neutral-400 hover:text-primary hover:bg-neutral-100 dark:hover:bg-neutral-800'
                                            }`}
                                        aria-label={link.name}
                        ...
                        return (
                            <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="p-2 rounded-lg text-neutral-500 dark:text-neutral-400 hover:text-primary hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors duration-200"
                                aria-label={link.name}
                            >
            ...
            {interestTags.length > 0 && (
                <div className="mb-6">
                    <h3 className="text-xs font-semibold text-neutral-400 dark:text-neutral-500 uppercase tracking-wider mb-2 text-center">
                        Interests
                    </h3>
                    <div className="flex flex-wrap justify-center gap-2">
                        {interestTags.map((tag, index) => (
                            <span
                                key={index}
                                className="px-2.5 py-1 text-xs rounded-full bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400 border border-neutral-200 dark:border-neutral-700"
                            >
                                {tag}
                            </span>
                        ))}
